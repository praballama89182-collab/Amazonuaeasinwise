...
# 1. Page Configuration - Wide layout for better data visibility
st.set_page_config(page_title="AMAZON MASTER AUDIT", page_icon="🎯", layout="wide")

# 2. Maison Fragrance Reference (Ensures item names are visible even if sales are 0)
MAISON_REF = {
...
def clean_numeric(val):
    """Handles currency, commas, and hidden formatting in Amazon reports."""
...
def get_brand_logic(text):
    """Robust brand detection scanning for SKU prefixes and keywords."""
    text = str(text).upper()
    targets = {
...
def find_col(df, keywords, exclude=None):
    """Dynamically finds column names to handle brackets or varying formats."""
...
def load_data(file):
    """Supports CSV and all modern Excel formats."""
...
if ad_f and biz_f and inv_f:
    with st.spinner('Backtracking ASINs and syncing stock...'):
        # Load Data
        df_ad = load_data(ad_f)
        df_biz = load_data(biz_f)
        df_inv = load_data(inv_f)
        ...
        # Create Master ASIN-to-Brand map from Sales/Ad history
        ...
        # 6. Process Inventory (Strict SELLABLE filter)
        df_inv_sell = df_inv[df_inv[i_cond].astype(str).str.strip().str.upper() == 'SELLABLE'].copy()
        # Backtrack: Use map first, then SKU logic
        ...
        # 7. Clean and Aggregate Sales Data
    ...
    # Global Overview Metrics (ACOS/TACOS removed as requested)
    ...
    tabs = st.tabs(["📊 Brand Breakdown"] + list(BRAND_MAP.values()))
    ...
    st.info("Upload your reports (Ads, Business, and Inventory) to generate your verified audit dashboard.")
