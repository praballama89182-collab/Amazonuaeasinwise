...
# 1. Page Configuration - Wide layout for maximum data visibility
st.set_page_config(page_title="AMAZON MASTER AUDIT", page_icon="🎯", layout="wide")

# 2. Maison Fragrance Master List (Resolves missing Item Names and Stock Mapping)
MAISON_REF = {
...
# 3. Enhanced Brand Mapping Logic (Includes all Campaign & SKU patterns)
def get_brand_logic(text):
    text = str(text).upper()
    targets = {
        'Maison de l’Avenir': ['MAISON', 'MA_', 'MA |', 'JPP', 'CEB', 'PGN', 'VGA'],
        'Creation Lamis': ['LAMIS', 'CL_', 'CL |', 'CLP', 'CPL', '3DM', 'CLAM'],
        'Jean Paul Dupont': ['DUPONT', 'JPD', 'JPD |'],
...
def clean_numeric(val):
    """Deep clean of currency, commas, and hidden formatting."""
...
def find_col(df, keywords, exclude=None):
    """Finds column names dynamically, ignoring case, brackets, and extra spaces."""
...
ad_f = st.sidebar.file_uploader("1. Ad Report (Excel/CSV)", type=file_types)
biz_f = st.sidebar.file_uploader("2. Business Report (Excel/CSV)", type=file_types)
inv_f = st.sidebar.file_uploader("3. Inventory Report (.txt only)", type=["txt"])

if ad_f and biz_f and inv_f:
    with st.spinner('Synchronizing Data and Backtracking Brands...'):
        # Load Data
        ...
        ad_agg = df_ad.groupby(a_asin).agg({a_sales: 'sum', a_spend: 'sum'}).reset_index()
        
        # Join Inventory + Business + Ads
        ...
        # 9. FINAL RESOLUTION (Item Names & Brands)
        # Force Maison Names from list, fall back to Amazon Titles
        ...
        # Calculate Ratios (Kept in detailed tables only)
    ...
    ov1.metric("Business Sales", f"AED {t_rev:,.2f}")
    ov2.metric("Ad Sales", f"AED {t_ad:,.2f}")
    ov3.metric("Ad Spend", f"AED {t_sp:,.2f}")
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        master.to_excel(writer, sheet_name='Audit_Summary', index=False)
    st.sidebar.download_button("📥 Download Master Report", data=output.getvalue(), file_name="Amazon_Performance_Master.xlsx")

else:
    st.info("Please upload your Ad Report, Business Report, and Inventory file to generate the dashboard.")
