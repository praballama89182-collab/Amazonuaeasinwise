...
def find_robust_col(df, keywords, exclude=['acos', 'roas', 'cpc', 'ctr', 'rate', 'new-to-brand']):
    """Automatically strips hidden spaces from headers to prevent KeyErrors."""
...
st.info("Verified Data: Campaign-First View, Absolute Totals, and 30-Day Velocity (DRR)")
...
if ad_file and biz_file:
    def load_df(file):
        ...
        # Fix for the 'hidden space' issue - strip all column names immediately
    ...
    # 3. Aggregation Logic
    # Group by Campaign + ASIN for the table
    ...
    # Group by ASIN only for organic sales isolation
    ...
    def display_metrics_dashboard(df_seg, raw_ad_segment, raw_biz_segment):
        """Standardized 13-metric dashboard using RAW data sources for accuracy."""
        t_sales = raw_biz_segment[biz_sales_col].sum()
        a_sales = raw_ad_segment[ad_sales_col].sum()
        o_sales = t_sales - a_sales
        t_spend = raw_ad_segment[ad_spend_col].sum()
        t_imps = raw_ad_segment[ad_imps_col].sum()
        t_clicks = raw_ad_segment[ad_clicks_col].sum()
        t_orders = raw_ad_segment[ad_orders_col].sum()
        ...
        st.markdown("#### ⚡ Efficiency & Contribution Dashboard")
        ...
        e7.metric("Portfolio DRR", f"{(t_sales/30):,.2f}")

    with tabs[0]:
        st.subheader("Global Portfolio Summary")
        display_metrics_dashboard(table_df, ad_df_raw, biz_df_raw)
        st.divider()
    ...
    for i, brand in enumerate(sorted(BRAND_MAP.values())):
        with tabs[i+1]:
            ...
            # Precise brand filtering for raw accuracy
            raw_ad_b = ad_df_raw[ad_df_raw['Campaign Name'].apply(get_brand_robust) == brand]
            raw_biz_b = biz_df_raw[biz_df_raw['Brand'] == brand]
            
            if not b_data.empty:
                st.subheader(f"{brand} Metrics Overview")
                display_metrics_dashboard(b_data, raw_ad_b, raw_biz_b)
                st.divider()
                st.dataframe(b_data[cols].sort_values(by='Total Sales', ascending=False), hide_index=True, use_container_width=True)
            else:
                st.warning(f"No products detected for {brand}.")
    ...
    st.sidebar.download_button("📥 Download Master Report", data=output.getvalue(), file_name="Amazon_ASIN_Audit_Verified.xlsx", use_container_width=True)
else:
    st.info("Upload your Ad and Business reports to proceed with the audit.")
