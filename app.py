...
# 1. Maison Item Name Reference (Solves the "Missing Name" issue)
MAISON_REF = {
...
BRAND_MAP = {'MA': 'Maison de l’Avenir', 'CL': 'Creation Lamis', 'JPD': 'Jean Paul Dupont', 
...
if ad_f and biz_f and inv_f:
    with st.spinner('Syncing data and resolving Maison names...'):
        ...
        # 1. Inventory Aggregation (Sellable)
        ...
        merged['Item Name'] = merged.apply(lambda r: MAISON_REF.get(r['Final_ASIN'], r[b_title] if r[b_title] != 0 else "N/A"), axis=1)
    ...
    ov3.metric("ACOS / TACOS", f"{(t_sp/t_ad if t_ad > 0 else 0):.1%} / {(t_sp/t_rev if t_rev > 0 else 0):.1%}")
    ov4.metric("Sellable Stock", f"{merged['Stock'].sum():,.0f} Units")
    ...
    with tabs[0]:
        ...
        st.dataframe(brand_summary.sort_values(by=b_sales, ascending=False).style.format({b_sales: '{:,.2f}', a_sales: '{:,.2f}', a_spend: '{:,.2f}', 'Stock': '{:,.0f}'}), use_container_width=True, hide_index=True)

    for idx, brand_name in enumerate(BRAND_MAP.values(), start=1):
        with tabs[idx]:
            b_data = merged[merged['Brand'] == brand_name]
            st.dataframe(b_data[['Final_ASIN', 'Item Name', 'Stock', b_sales, a_sales, 'ACOS', 'TACOS']].sort_values(by=b_sales, ascending=False).style.format({b_sales: '{:,.2f}', a_sales: '{:,.2f}', 'ACOS': '{:.1%}', 'TACOS': '{:.1%}', 'Stock': '{:,.0f}'}), use_container_width=True, hide_index=True)
