...
if ad_file and biz_file:
    ...
    # 2. Clean Metric Values for All Data
    ...
    def display_metrics_dashboard(raw_ad_seg, raw_biz_seg):
        """Unified 12-metric dashboard excluding DRR for overview headers."""
        ...
        t_imps = raw_ad_seg[ad_imps_col].sum()
        t_clicks = raw_ad_seg[ad_clicks_col].sum()
        t_orders = raw_ad_seg[ad_orders_col].sum()
    ...
    for i, brand in enumerate(sorted(BRAND_MAP.values())):
        with tabs[i+1]:
            # Precise brand filtering for dashboard accuracy
